    ...
    <div className="min-h-screen bg-stone-900 text-stone-300 p-6 font-mono selection:bg-emerald-500/30">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          <StatCard
            icon={<Package size={18} className="text-stone-400" />}
            label="Total Orders"
            value={orders.length}
          ...
          {filteredOrders.length === 0 ? (
            <div className="p-10 border border-dashed border-stone-700 text-center text-stone-500 rounded-xl">
              {search || statusFilter !== 'ALL'
                ...
                : 'No database entries found. Place a test order to get started.'}
            </div>
          ) : (
                  ...
                  <div className="text-xs grid sm:grid-cols-2 gap-1.5 text-stone-400">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        ...
                        className="text-emerald-600 hover:text-emerald-400 text-[10px] font-bold transition-colors"
                      >
                        WA
                      </a>
                    </div>
                    <div className="sm:col-span-2 text-stone-500">
                      📍 {order.addressLine}, {order.city}, {order.district}, {order.province}
                    </div>
                  </div>
                </div>

                {/* Actions panel */}
                <div className="w-full lg:w-auto shrink-0 bg-stone-900 p-4 rounded-xl border border-stone-800">
        ...
        <p className="text-center text-stone-700 text-[10px] mt-10 font-mono">
          AL GHAZI WOOD CRAFTS — INTERNAL SYSTEM v2.0 — {new Date().getFullYear()}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-stone-950 p-5 rounded-2xl border border-stone-800">
      <div className="flex items-center gap-2 mb-2">
        {icon}
        <span className="text-[9px] uppercase font-bold text-stone-500 tracking-wider leading-tight">
          {label}
        </span>
      </div>
